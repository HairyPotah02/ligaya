...
async def check_or_set_log_channel():
    try:
        ...
        if al_log_channel:
            ...
            log_channel = await NEXAUB.create_channel(title="Ligaya Userbot Logs", description="Logs of your Ligaya Userbot")
            log_channel_id = log_channel.id
            await NEXAUB.set_chat_photo(chat_id=log_channel_id, photo="cache/Ligaya2.jpg")
            welcome_to_nexaub = f"""
**Welcome to Ligaya Userbot**
Thanks for trying Ligaya Userbot. If you found any error, bug or even a Feature Request please report it at **@EngrJuanMala**
 ...
 **~ Ligaya Userbot, Developers**"""
        ...
        logging.warn(
            f"Error: \n{e} \n\nPlease check all variables and try again! \nReport this with logs at @EngrJuanMala if the problem persists!")
        exit()
...
async def download_plugins_in_channel():
    ...
    if plugin_channels:
        ...
        logging.info(
            "No Custom Plugin Channels were specified, Ligaya-Userbot is running with default plugins only!")
