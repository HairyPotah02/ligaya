...
async def pyroalive(_, message: Message):
    ...
    custom_alive_msg = get_alive_msg if get_alive_msg else "Heya, I'm Using Ligaya Userbot"
    # Alive Pic
    g_al_pic = await get_custom_var(var="ALIVE_PIC")
    alive_pic = g_al_pic[1] if g_al_pic else "cache/Ligaya2.jpeg"
...
**✨ Ligaya UserBot is Alive**
    
    **》 Ligaya Userbot Version:** `{NEXAUB_VERSION}`
...
async def pingme(_, message: Message):
    ...
    await ping_msg.edit(f"**Pong:** `{ping_time} ms` \n\n ~ **✨ Ligaya-Userbot**", disable_web_page_preview=True)
...
async def get_alive_pic(_, message: Message):
    ...
    if g_al_pic:
        ...
        if ptype == "gif":
            await NEXAUB.send_animation(chat_id=message.chat.id, animation=picture, caption=f"**Ligaya-Userbot's Custom Alive Picture** \n\n**Type:** `{ptype}`")
        else:
            await NEXAUB.send_photo(chat_id=message.chat.id, photo=picture, caption=f"**Ligaya-Userbot's Custom Alive Picture** \n\n**Type:** `{ptype}`")
    else:
